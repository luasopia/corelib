...
if _Gideros then
    ...
    local function tchMove(self, event) --printf('%s touch move', self.name)
        ...
        if self.__bd:hitTestPoint(t.x, t.y) then
            ...
            self:touch{id=t.id, phase='cancel', x=t.x, y=t.y, dx=0, dy=0}
            self.__tch[t.id] = nil
            event:stopPropagation()
        end
    end
...
elseif _Corona then
    ...
    -- bd가 몇단계든지 그룹안에 포함되었다고 해도 이 메서드는 스크린xy값을 반환한다.
    ...
    local function tch(e)
        ...
        elseif e.phase == 'moved' then print('tch move event')
          ...
          if hitTestFunction(self.__bd, e.x, e.y) then
            ...
            self:touch{id=e.id, phase='cancel', x=e.x, y=e.y, dx=dx, dy=dy}
            self.__tch[e.id] = nil
            return true
          end
        ...
        elseif e.phase == 'ended' then print('tch end')
          ...
          _Corona.display.getCurrentStage():setFocus(nil)
          self.__bd.isFocus = false
          self:touch{id=e.id, phase='end', x=e.x, y=e.y, dx=0, dy=0}
          self.__tch[e.id] = nil
          return true
